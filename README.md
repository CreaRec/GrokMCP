...
**Env:** reuse downloader `settings.json` (`TELEGRAM_SETTINGS_PATH` + `TELEGRAM_USER_ID`) or `TELEGRAM_API_ID` / `TELEGRAM_API_HASH` / `TELEGRAM_SESSION`; `FINDVID_BOT_USERNAME`, `DOWNLOADER_BOT_USERNAME`; port **8801**. VIP/rate-limits/UI changes on Findvid can break automation.
