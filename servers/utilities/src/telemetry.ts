...
const SERVICE_NAME = "utilities";
const SERVICE_NAMESPACE = "mcp";

export type ToolResult = "success" | "error";

export type McpErrorType = "dashboard" | "auth" | "timeout" | "network" | "unknown";
...
export function classifyError(err: unknown): McpErrorType {
  ...
  if (/dashboard|utilities|non-json|unexpected payload|invalid json/i.test(message)) {
    return "dashboard";
  }
