...
export function classifyError(err: unknown): McpErrorType {
  ...
  if (err && typeof err === "object" && "name" in err) {
    ...
    if (name === "DateRangeError" || name === "AccessUrlError") {
      return "validation";
    }
    if (name === "SimplefinApiError") {
      return "simplefin";
    }
  }
  const message = errorMessage(err);
  if (/ETIMEDOUT|ESOCKETTIMEDOUT|timeout/i.test(message)) return "timeout";
  if (/exceeds SimpleFIN limit|Invalid date|Invalid range|SIMPLEFIN_ACCESS_URL/i.test(message)) {
    return "validation";
  }
  if (/SimpleFIN|account set|non-json|invalid json/i.test(message)) {
    return "simplefin";
  }
  if (/unauthorized|auth|401|403|forbidden/i.test(message)) return "auth";
  if (/ECONNREFUSED|ENOTFOUND|network|fetch failed|unreachable/i.test(message)) {
    return "network";
  }
  return "unknown";
}
