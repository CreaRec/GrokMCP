...
describe("classifyError", () => {
  ...
  it("classifies simplefin, validation, and network errors", async () => {
    const { classifyError } = await import("./telemetry.js");
    expect(classifyError(new Error("SimpleFIN API returned invalid JSON"))).toBe(
      "simplefin",
    );
    expect(classifyError(new Error("Date range exceeds SimpleFIN limit of 90 days"))).toBe(
      "validation",
    );
