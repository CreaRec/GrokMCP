...
const SERVICE_NAME = "synology";
const SERVICE_NAMESPACE = "mcp";

export type ToolResult = "success" | "error";

export type McpErrorType = "database" | "embedding" | "timeout" | "network" | "unknown";
...
export function classifyError(err: unknown): McpErrorType {
  ...
  if (/database|postgres|prisma|sql|query/i.test(message)) return "database";
  if (/embed|onnx|transform/i.test(message)) return "embedding";
